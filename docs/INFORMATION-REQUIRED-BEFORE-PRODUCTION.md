...
- **Production URL for the employer requirement form**
  ...
  - Blocks: **every conversion on the site.** `src/data/company.ts` sets `requirementFormUrl` to `https://www.directhired.com/employer-requirement`, which does not resolve. That URL is behind **46 calls to action across all eight built pages** — every "Submit Your Requirements" button in the header, the mobile nav, every hero, every closing block and the fixed mobile bar, plus the 404. Nothing on the site is broken to look at and every page passes every check; the primary conversion path simply ends nowhere. DirectHired confirmed on 2026-08-16 that the form stays on their existing separate site, so what is needed is **that site's live form URL** — not a form to be built.
