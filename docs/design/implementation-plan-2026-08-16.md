...
| **C-47** | Rewrite step 2's label. Steps 1 and 2 read "Understand your family" then "Understand your needs" — adjacent, same opening verb, and the distinction is not self-evident. Either merge them into one step, or rename step 2 to name what it actually covers. | Two consecutive steps opening on the same verb makes a five-step process look like one step split to reach five. | `src/sections/Process.astro:22`, `:26` | P1 | LOC | Taste P1-2 — **D-4 ANSWERED; DONE** (merged into "Understand your household needs", `8159410`) |
...
| **C-49** | Revisit `grid-template-columns: 3fr 2fr` once C-33's copy exists — if the second card gains real content, an equal split may read better. | The unequal split was chosen to reflect content weight; that reasoning changes when the content changes. | `src/sections/PricingSection.astro:131-140` | P2 | LOC | Taste P1-4 — **DONE**: `1fr 1fr`, once D-6 made both cards itemised |
...
| **C-57** | Add an optional explanatory field to `TotalOnlyPackage` (e.g. `readonly note: string \| null`) so C-33 has somewhere honest to read from. The discriminated union must keep making an invented `lineItems` a **type error**. | Preserves the compiler guarantee that is the point of the type. | `src/data/pricing.ts:17-23` | P1 | LOC | Taste P1-4 — ~~blocked, D-6~~ **MOOT**: the package is itemised, so there is no total-only card to annotate. `TotalOnlyPackage` stays in the union as the guard against a future invented breakdown |
...
**Items:** C-56, C-57 (moot — see D-6).
**Effort:** 1–2 h. C-57 is moot — D-6 was answered with a real breakdown, so there is no total-only card to annotate.
...
> **Answered so far: D-4, D-5, D-6 (DirectHired, 2026-08-16). 9 outstanding.**
> An answer is not a change. Each of the three below unblocked plan items that
> had been *correctly* deferred, and those items do not implement themselves —
> three of them shipped as visible defects a client spotted by eye before anyone
> swept the plan for what the answer had unblocked. **When a decision is
> answered, record it in the row below, then re-read every item that names it.**
> The rows carry their answer, its date, and what it unblocked, so the sweep is
> possible from this document alone.
...
| **D-4** ✅ **ANSWERED 2026-08-16** | Approve new wording for process step 2. (C-47) | Brief §23 constrains process wording and notes the process may still be refined. | Recommend merging steps 1 and 2 into one step if the real process supports it; otherwise relabel step 2 to what it actually covers. **DirectHired must supply the wording** — inventing a process step is a §78 violation.<br><br>**ANSWER:** merge steps 1 and 2 into one step, **"Understand your household needs"**. <br>**Unblocked and done:** C-47 (`Process.astro`, commit `8159410`); the `how-matching-works` FAQ, which lists the same steps; the section lede, now *derived* from `steps.length` rather than typed, so "five steps" cannot go stale again; and the `.process-rail` 80em justification, whose arithmetic was measured with five steps and has been recomputed with four. The master brief still describes five steps at §665/§813 and is superseded on this point. |
| **D-5** ✅ **ANSWERED 2026-08-16** | Supply **one distinguishing fact per helper source** (Indonesia / Myanmar / Mizoram), or approve deleting the summaries. (C-53, C-54) | Requires business knowledge nobody in the codebase has. | **Supply the facts if they exist; otherwise delete.** Three identical sentences say less than no sentence, and this is the page's clearest "AI-generated template" tell.<br><br>**ANSWER:** **"No real difference."** Same service, same package, same matching process; only the source country differs. There is no fact to wait for.<br>**Unblocked and done:** C-53 — the three `summary` fields were deleted, and `HelperSources.astro` now states the equivalence **once**, above the three names, so the block reads as a decision rather than as content that failed to load; C-54 — the three markdown bodies, previously the same paragraph with the country swapped, now say the same true thing rather than pretending to be per-country; the `source-countries` FAQ, which promised "more on each"; and C-62 — the declared input that was going to be raised for this is recorded as answered instead of added. |
| **D-6** ✅ **ANSWERED 2026-08-16** | Supply explanatory copy for the Fly-In **Without Replacement** package — what it covers and why it is not itemised. (C-33, C-57) | Already a tracked Category C declared input. | Supply it. Without it the card stays visually broken (85px against 310px) and the block whose job is transparency reads as failing to load. **Do not** approve a fabricated breakdown.<br><br>**ANSWER:** better than asked for — DirectHired supplied the **real breakdown**, which also corrected the total from **$1,252.10 to $1,140.10**. The package is `itemised`, not `total-only`.<br>**Unblocked and done:** C-33 and C-57 are **moot** — the card has six real line items, so it needs neither explanatory copy nor a `note` field; C-49 — the grid is `1fr 1fr`, because `3fr 2fr` described a content weight that no longer exists; both cards state their replacement term (`"No replacement"` rather than a blank); labels harmonised to `"Handling & transport"` on both. The gap between the packages is **$500** (the agent fee), not the **$388** the master brief states at §508 — the brief is stale on this and `tests/pricing.test.ts` asserts $500 against it. |
...
| **WS-3** Sections | 22 | 12–16 h | WS-0, WS-4. ~~C-53 on D-5; C-47 on D-4~~ — both answered 2026-08-16 and implemented | After WS-4 merges |
| **WS-4** Primitives | 4 | 3–4 h | WS-0. ~~C-33 on D-6~~ — answered 2026-08-16; C-33 is moot | After WS-0 merges |
| **WS-5** Content & data | 2 | 1–2 h. ~~C-57 on D-6~~ — answered 2026-08-16; C-57 is moot | — | Immediately |
| **WS-6** Docs | 5 | 6–8 h | C-61 wants WS-2's C-22 first | Immediately |

**Total engineering: ≈ 35–50 hours**, plus client inputs (D-1…D-12; **D-4, D-5 and D-6 answered 2026-08-16 — 9 outstanding**) and a half-day photo shoot. With four implementers and the dependency graph above, the critical path is WS-0 → WS-4 → WS-3, roughly **four to five working days**.
