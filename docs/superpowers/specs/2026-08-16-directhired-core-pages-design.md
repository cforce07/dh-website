...
- **2.6.5 — The requirements form is a separate APPLICATION on the SAME domain.** *Answered 2026-08-16; corrected 2026-08-17 when the URL arrived.* DirectHired: *"Yes it's a different site. We can audit design on that later."* The employer requirement form is not migrating into this build, and its design is outside this project; a design audit of it is deferred by DirectHired's instruction.

  **WHAT THIS ENTRY SAID AND WHY IT WAS WRONG.** It read *"The requirements form stays on the legacy site"* and drew the consequence that **"the primary conversion path leaves this domain, so conversion cannot be measured from here."** DirectHired supplied the address on 2026-08-17: `https://www.directhired.com/app/requirements` — the **same host** as this site, served through the **same CloudFront distribution**, from a different S3 origin behind it. Separate application, same domain. The consequence therefore does not follow: a visitor clicking a primary CTA stays on `www.directhired.com`, and measuring the path is same-origin work that is available whenever anyone wants it, rather than an impossibility to be accepted. Nothing is instrumented today, deliberately; what changed is the constraint, not the site.

  **The inference was the error, not the answer.** "A different site" is a perfectly ordinary way to describe an application you did not build in the same codebase, and it was read here as "a different domain". Recorded in full rather than silently corrected because the same conclusion was carried into `docs/OPEN-DECISIONS.md` (§ *Housekeeping*) and into the cutover runbook, and because an assumption about origins is exactly the kind that gets re-derived from an old spec line. `src/data/company.ts` records what was verified about the URL before it was used; `tests/company.test.ts` records why no host assertion was added on the strength of the new fact.
...
- **2.6.7 — The "not asked to put your family into a category" clause is deleted.** *Answered 2026-08-16.* `/why-directhired`'s Personalised Service pillar claimed *"…you are not asked to put your family into a category before anyone has spoken to you."* That is a claim about the legacy employer-requirement form (§2.6.5) — outside this project, unseen by anyone in this repository, and one click from this page's primary CTA. **DirectHired chose to delete it** rather than defend it. It is gone from the page; `src/pages/why-directhired.astro` records the exact wording and the one condition for restoring it — written confirmation of what fields that form asks for.

  *Corrected 2026-08-17.* This entry also said the form sat "behind a URL that 404s today". It no longer does (§2.6.5). That strengthens the deletion rather than reopening it: the form is now genuinely reachable in one click from this pillar, so a sentence describing fields nobody here has read would be falsifiable by any visitor. The restoring condition is unchanged and still unmet — reaching the form was never the obstacle, being told in writing what it asks is.
